...
RESOLUTIONS=("test")
CSV_FILE="${RESULTS_DIR}/all_fps_${VERSION}_rt.csv"
...
run_benchmark() {
    ...
    for res in "${RESOLUTIONS[@]}"; do
        ...
        # Build the command
        cmd="uv run evaluate_traced_optix.py --output_path /tmp/scene --dataset_path ${scene_path} --resolution ${res}" # Use the new resolution flag
