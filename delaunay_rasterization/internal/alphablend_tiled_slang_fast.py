...
class AlphaBlendTiledRenderFast(torch.autograd.Function):
    @staticmethod
    def forward(ctx,
        ...
        fast_shader = shader_manager.get_fast(render_grid.tile_height, render_grid.tile_width, aux_dim)

        # Forward kernel (non-differentiable, plain TensorView)
        fwd_kernel = fast_shader.splat_tiled_fast_fwd(
    ...
    def backward(ctx, grad_output_img, grad_xyzd_img, grad_distortion_img, grad_vert_alive):
        ...
        fast_shader = shader_manager.get_fast(render_grid.tile_height, render_grid.tile_width, ctx.aux_dim)

        tet_alive = torch.zeros((indices.shape[0]), dtype=bool, device=vertices.device)

        # Backward kernel with explicit grad tensors
        bwd_kernel = fast_shader.splat_tiled_fast_bwd(
            sorted_gauss_idx=sorted_tetra_idx,
            tile_ranges=tile_ranges,
            indices=indices,
            vertices=vertices,
            ...
            tet_alive=tet_alive,
            ray_jitter=ray_jitter,
            grad_output_img=grad_output_img,
            grad_xyzd_img=grad_xyzd_img,
            grad_distortion_img=grad_distortion_img,
            vertices_grad=vertices_grad,
            cell_values_grad=cell_values_grad,
            tcam=tcam,
        )

        bwd_kernel.launchRaw(
            blockSize=(render_grid.tile_width,
                       render_grid.tile_height, 1),
            gridSize=(render_grid.grid_width,
                      render_grid.grid_height, 1)
