...
M = 500  # total frames
N = 10  # number of points per frame
device = 'cpu'
...
# Dummy data: each frame contains N 3D points (replace with your clipped_circumcenters)
...
centers = 2 * torch.rand((N, 3), device='cpu') - 1
centers[:, 2] += 5
...
### Create motion:
# Save initial positions as centers
centers = 2 * torch.rand((N, 3), device=device) - 1
centers[:, 2] += 5

# Choose random radii for circles (for instance, between 0.1 and 0.5)
radii = 0.1 + 0.4 * torch.rand((N,), device=device)
...
vertices = centers
thetas = torch.ones((N, 1), device=device)
...
for i in range(M):
    ...
    scaling = 5e+1/sensitivity.reshape(-1, 1).clip(min=1)/ radii.reshape(-1, 1)
